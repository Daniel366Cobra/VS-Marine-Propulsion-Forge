...
public class BallastTankHorizontalCTBehavior extends ConnectedTextureBehaviour.Base {
    ...
    public BallastTankHorizontalCTBehavior(CTSpriteShiftEntry layerShift, CTSpriteShiftEntry topShift, CTSpriteShiftEntry innerShift) {
    ...
    public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
        if (sprite != null && direction.getAxis() == state.getValue(AXIS) && innerShift.getOriginal() == sprite)
            return innerShift;
        return direction.getAxis() == state.getValue(AXIS) ? topShift : layerShift;
    }

    @Override
    protected Direction getUpDirection(BlockAndTintGetter reader, BlockPos pos, BlockState state, Direction face) {
        Direction.Axis vesselAxis = state.getValue(AXIS);
